...
int8_t IAP::write(uint8_t *bytes, uint16_t length){
	ESP_LOGD(this->LOG_TAG, "write: bytes %p and len %u", bytes, length);
	...
	uint16_t numberOfBytesToCopy;
	while(length > 0){
		spaceRemaining = IAP::PAGE_SIZE - this->pageBufferIndex;
		numberOfBytesToCopy = (spaceRemaining < length) ? spaceRemaining : length;
...
int8_t IAP::writePageBuffer(){
	ESP_LOGD(this->LOG_TAG, "Writing pageBuffer");
	if(this->pageBufferIndex == 0) return 0;

	ESP_LOGD(this->LOG_TAG, "write pageBuffer: writing %u bytes to address 0x%08x", this->pageBufferIndex, this->currentFlashAddress);
