...
int8_t Local::waitForMessages(uint16_t timeout){
	...
	while(((millis() - millisAtStart)/1000 < timeout) || (timeout == __UINT16_MAX__)){
...
int8_t Local::waitForMessagesInMilliseconds(uint32_t timeout){
	int8_t returnValue = 0;
	uint32_t millisAtStart = millis();
	while(((millis() - millisAtStart) < timeout) || (timeout == __UINT32_MAX__)){
		returnValue = this->readPacket();
		if(returnValue != 0){
			return returnValue;
		}
