...
System::wakeUpCause_t System::wakeUpCause = WUC_UNDEF;
...
void System::setWakeUpCause()
{
	switch(esp_sleep_get_wakeup_cause())
	{
		case ESP_SLEEP_WAKEUP_EXT0:
		{
		  System::wakeUpCause = WUC_BUTTON_L1;
		} break;
		case ESP_SLEEP_WAKEUP_EXT1:
		{
			uint64_t wkUpState = esp_sleep_get_ext1_wakeup_status();
			if      (wkUpState & (1ULL << BUTTON_L2)) System::wakeUpCause = WUC_BUTTON_L2;
			else if (wkUpState & (1ULL << BUTTON_L3)) System::wakeUpCause = WUC_BUTTON_L3;
			else if (wkUpState & (1ULL << BUTTON_R1)) System::wakeUpCause = WUC_BUTTON_R1;
			else if (wkUpState & (1ULL << BUTTON_R2)) System::wakeUpCause = WUC_BUTTON_R2;
			else if (wkUpState & (1ULL << BUTTON_R3)) System::wakeUpCause = WUC_BUTTON_R3;
			else System::wakeUpCause = WUC_UNDEF;
		} break;
		case ESP_SLEEP_WAKEUP_TIMER:
		{
		  System::wakeUpCause = WUC_INT_RTC;
		} break;
		case ESP_SLEEP_WAKEUP_UNDEFINED:
		{
			if (button5Pressed && button6Pressed) System::wakeUpCause = WUC_DEBUG_MODE_REQUEST;
			else if (button6Pressed) System::wakeUpCause = WUC_AP_MODE_REQUEST;
			else if (button4Pressed) System::wakeUpCause = WUC_UPDATE_REQUEST;
			else if (button5Pressed) System::wakeUpCause = WUC_PAIRING_REQUEST;
			else System::wakeUpCause = WUC_UNDEF;
		} break;
		default:
		{
			System::wakeUpCause = WUC_UNDEF;
		}
	}
}
