...
void Audio::tone(uint16_t frequency, uint32_t duration)
{
	...
	if(isToneBlocking && duration < AudioOutput::PLAY_FOREVER) {
		vTaskDelay((duration+20)/portTICK_PERIOD_MS);	// 20 ms delay between tones
	}
}
...
{
	const uint32_t RESERVED_RAM = 60000;	// found empirically to prevent allocation errors
	...
	ESP_LOGD(this->LOG_TAG, "free heap = %d", heap_caps_get_free_size(MALLOC_CAP_DEFAULT));
	if (((int32_t)(heap_caps_get_free_size(MALLOC_CAP_DEFAULT))-RESERVED_RAM) > micDataSize)
	{
		micData = (uint8_t*)malloc(micDataSize);
		if (micData == NULL)
		{
			ESP_LOGE(this->LOG_TAG, "allocating micData failed!");
			return;
		}
	}
	else
	...
	audioOut->stop();
	vTaskDelay(50/portTICK_PERIOD_MS);	// extra delay to improve stability
	audioOut->deinit();
	...
	vTaskDelay(milliSeconds/portTICK_PERIOD_MS);	// block the programm while recording
	microphone->deinit();
...
void Audio::playRecorded(bool playForever)
{
	...
	if (sampleRate_ != microphone->getSampleRate())
	{
		...
		vTaskDelay((micDataSize*1000/sampleRate_+50)/portTICK_PERIOD_MS);
	...
	uint16_t micSampleRate)
{
	...
	audioOut->deinit();
	microphone->init(micSampleRate, updateRate_);
	vTaskDelay(50/portTICK_PERIOD_MS);	// additional delay needed to not instantly detect a loud noise
