...
class AutoTurnOff{
	...
	public:
		...
		void configure(uint16_t seconds = DEFAULT_AUTO_TURN_OFF_SECONDS,
			uint8_t buttonByte = System::DEFAULT_TURN_OFF_BUTTON_BYTE);
		bool isTriggered();
		void reset();
	...
	private:
		...
		/* AutoTurnOff defines */
		static const uint16_t DEFAULT_AUTO_TURN_OFF_SECONDS	= 120;
		static const uint8_t AUTO_TURN_OFF_TIMER_NR					= 0;

		const char *LOG_TAG = "AutoTurnOff";
		System *system = NULL;
		uint16_t autoTurnOffSeconds = DEFAULT_AUTO_TURN_OFF_SECONDS;
		uint8_t autoTurnOffButtonByte = System::DEFAULT_TURN_OFF_BUTTON_BYTE;
