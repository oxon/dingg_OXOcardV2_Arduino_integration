...
static void handle_animations_task(void *pv){
	PairingScenario *ps = (PairingScenario *)pv;
	vTaskDelay(5000/portTICK_PERIOD_MS);
	ps->oxocard->matrix->setForeColor(oxocardBrandColors[1]);
	while(1){
		ps->wlanIconAnimation();
		if (ps->deleteAnimationsTaskRequest){
			ps->animationsHandle = NULL;
			vTaskDelete(ps->animationsHandle);
		}
	}
}
...
int8_t PairingScenario::configure(void){
	...
	xTaskCreate(&handle_animations_task, "pair_animations", 2048, (void*)this, 1, &this->animationsHandle);
...
void PairingScenario::deleteAnimationsTask(){
