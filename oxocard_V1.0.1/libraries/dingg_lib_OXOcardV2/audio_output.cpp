...
void AudioOutput::init(uint16_t sampleRate, uint16_t updateRate)
{
	...
	i2s_zero_dma_buffer(I2S_PORT_NUM);
	vTaskDelay(updateRate/portTICK_PERIOD_MS);	// wait until at least one dma_buf is filled
...
void AudioOutput::stop()
{
	...
	vTaskDelay(updateRate_/portTICK_PERIOD_MS);	// wait long enough that a running tone_task can finish
}
