...
HTTPSResponse *Backend::doGETRequest(const char *fmt, ...){
	va_list list;
	size_t count = 0;
	for(size_t i=strlen(fmt)-1; i>0; i--){
		if(fmt[i] == '%' && fmt[i+1] != '%')
			count++;
	}
	va_start(list, fmt);
...
int8_t Backend::flash(){
	...
	size_t len = strlen("oxocard/flash?id=")+this->baseURLLength+strlen(id);
	char url[len+1] ={0};
	snprintf(url, len, "%s%s%s", this->baseURL, "oxocard/flash?id=", id);

	HTTPSOTA *sota = new HTTPSOTA(url);
	return sota->flash();
}
